...
jobmanagerDB = {
	...
	addJob: function( clientId, description, hourPrice ) {
		...
		statement.executeAsync({
          ...
          jobmanagerDB.startJob(jobmanagerDB._connection.lastInsertRowID);
				}
			}
		});
	},
	...
	_finishUnfinishedJobPeriods: function( jobId, callback ){
		try{
			...
			statement.executeAsync({
              ...
              aReason == Components.interfaces.mozIStorageStatementCallback.REASON_ERROR) {
						jobmanagerDB._callbackCall( callback, [ false ] );
					}
					else if(aReason == Components.interfaces.mozIStorageStatementCallback.REASON_FINISHED) {
						jobmanagerDB._callbackCall( callback, [ true ] );
					}
				}
			});
